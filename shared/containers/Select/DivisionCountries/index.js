...
const DivisionCountriesSelect = ({
  ...
  isShowOnlyOperationalCountries = false,
}) => {
  const dispatch = useDispatch();
  const divisionCountriesSelector = isShowOnlyOperationalCountries ?
    getDivisionsCountriesSelector.getOperationalCountries :
    getDivisionsCountriesSelector.getData;
  const divisionCountries = useSelector(divisionCountriesSelector);
  ...
  const getSelectedDivisionCountryOrCountries = () => {
    if (mode === 'multiple') {
      return defaultSelectedDivisionCountries;
    }
    // single select
    if (localSelectedDivisionCountries === null) {
      // if default value
      return defaultSelectedDivisionCountry;
    }
    // if user clear country selection
    return undefined;
  };

  const DEBOUNCE_TIME = 200;
  const debouncedSelectedDivisionCountries = useDebounce(localSelectedDivisionCountries, DEBOUNCE_TIME) || getSelectedDivisionCountryOrCountries();
