...
const FoodPromoSelect = ({ disabled, hasMultiple, fieldName, rules, form, inline }) => {
  const dispatch = useDispatch();
  const { t } = useTranslation('marketing');
  const [dropdownVisibility, setDropdownVisibility] = useState(false);

  const isPending = useSelector(promoSelector(PROMO_TYPE.FOOD_PROMO).getIsPending);
  const foodPromoList = useSelector(promoSelector(PROMO_TYPE.FOOD_PROMO).getData);

  useEffect(() => {
    dispatch(Creators.initContainer());
    // Get & init label infos for selected promos
    if (form?.getFieldValue(fieldName)) {
      dispatch(Creators.getPromoDetailsByTargetDomainRequest({
        promoType: PROMO_TYPE.FOOD_PROMO,
  ...
  const searchPromo = searchString => {
    if (searchString.length >= 3) {
      dispatch(Creators.getPromosByTargetDomainRequest({
        promoType: PROMO_TYPE.FOOD_PROMO,
        filterParams: getPromoQueryParamsForTargetDomain(PROMO_TYPE.FOOD_PROMO, searchString),
      }));
    }
  };
  ...
  return (
    <Form.Item
      name={fieldName}
      label={inline ? null : t('FOOD_PROMO')}
      className="d-inline"
      rules={rules}
    >
      <Select
...
export default FoodPromoSelect;
