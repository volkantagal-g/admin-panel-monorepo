...
const url = '/algorithm/config/domain/market/list';

const tableCellInDocument = (container, text) => {
  return Array.from(container).some(cell => within(cell).queryByText(text));
};

describe('Algorithm Market Domain Config List Page', () => {
  it('should render algorithm market domain config list page without error', async () => {
    await renderPage({
      pagePermKey: permKey.PAGE_ALGORITHM_MARKET_DOMAIN_CONFIG_LIST,
      pageUrl: url,
