...
const url = '/algorithm/config/domain/locals/list';

const tableCellInDocument = (container, text) => {
  return Array.from(container).some(cell => within(cell).queryByText(text));
};

describe('Algorithm Locals Domain Config List Page', () => {
  it('should render algorithm locals domain config list page without error', async () => {
    await renderPage({
      pagePermKey: permKey.PAGE_ALGORITHM_LOCALS_DOMAIN_CONFIG_LIST,
      pageUrl: url,
