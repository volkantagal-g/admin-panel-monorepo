...
const url = '/algorithm/config/domain/voyager/list';

const tableCellInDocument = (container, text) => {
  return Array.from(container).some(cell => within(cell).queryByText(text));
};

describe('Algorithm Voyager Domain Config List Page', () => {
  it('should render algorithm voyager domain config list page without error', async () => {
    await renderPage({
      pagePermKey: permKey.PAGE_ALGORITHM_VOYAGER_DOMAIN_CONFIG_LIST,
      pageUrl: url,
