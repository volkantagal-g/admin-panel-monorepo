...
const newScoreMapping = { name: { tr: '', en: '' }, min: null, max: null };

const MultiChoiceMapping = ({ data, isPending, tabKey }) => {
          ...
          allScores.map((value, index) => (
            ...
            <Row gutter={[20, 4]} align="middle" key={index}>
              ...
              <Col lg={2} xs={2}>
                ...
                <InputWrapper
                  mode="number"
                  inputKey={[type, index, 'min']}
                  value={get(values, `${type}.${index}.min`)}
                  isTouched={get(touched, `${type}[${index}].min`)}
                  hasError={get(errors, `${type}[${index}].min`)}
              ...
              <Col lg={2} xs={2}>
                ...
                <InputWrapper
                  mode="number"
                  inputKey={[type, index, 'max']}
                  value={get(values, `${type}.${index}.max`)}
                  isTouched={get(touched, `${type}[${index}].max`)}
                  hasError={get(errors, `${type}[${index}].max`)}
                  setFieldValue={setFieldValue}
                  disabled={isPending || !isFormEditable}
                />
              </Col>
...
export default MultiChoiceMapping;
