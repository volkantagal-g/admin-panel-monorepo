...
import { crisisIncidentList, crisisIncidentDetails } from '@shared/api/courierCrisis/index.mock.data';
...
describe('Courier Crisis Management: Incident', () => {
  let renderResult;
  let origWindowOpen;
  beforeAll(() => {
  ...
  it('is present in the page', async () => {
    renderResult = await renderComponent({ ui: <CrisisIncidents courierId="587c9e24a1a56d0014072d82" /> });
    expect(screen.getByTestId('courier-crisis-incident')).toBeInTheDocument();
  });
  ...
  it('shows grid controls WITH "New Card" when permissions are present', async () => {
    ...
    act(() => {
      addUserPermissions([permKey.PAGE_COURIER_DETAIL_CRISIS_MGMT_CREATE]);
    });
    const btnNewCard = control.getBtnNewCard();
  ...
  it('shows a populated grid content when filters are provided', async () => {
    const bringBtn = control.getBtnSearch();
    fireEvent.click(bringBtn);
    await waitFor(() => expect(bringBtn).toBeEnabled());
  ...
  it('shows the grid with delete button with permission', async () => {
    ...
    const { addUserPermissions } = renderResult;
    act(() => {
      addUserPermissions([permKey.PAGE_COURIER_DETAIL_CRISIS_MGMT_DELETE]);
    });
  ...
  it('deleting a crisis works fine', async () => {
    ...
    mockApiPerTestCase({
      url: `/courierCrisisManagement/deleteCard/${row._id}`,
      successData: {
        ...row,
