...
import { Creators } from '@app/pages/Banner/New/redux/actions';
import { COMPONENT_TYPE, FILE_UPLOAD_STATE_KEY } from '@app/pages/Banner/constants';

import { fileUploadSelector } from '@app/pages/Banner/New/redux/selectors';
import { initPlaceholderImage } from '@shared/utils/marketing/generatePlaceholderImage';
...
const ImageCarousel = ({ form, phoneLanguages, contentFieldNamePrefix, componentType, lang }) => {
  return (
...
const CarouselItem = ({ form, contentFieldNamePrefix, phoneLanguages, componentType, lang, carouselItem }) => {
  const dispatch = useDispatch();
  const isImageLoading = useSelector(fileUploadSelector.isBannerContentImagePending);
  const ref = useRef(null);
  useEffect(() => {
    initPlaceholderImage(carouselItem.width, carouselItem.height, ref);
  }, [carouselItem.height, carouselItem.width, componentType]);
  return (
            ...
            onOkayClick={(loadedImage, file, isAppliedToOtherLanguanges) => {
              ...
              dispatch(Creators.uploadFilesToS3Request({
                content: {
                  ...
                  fileLang: lang,
                  form,
                  fieldName: carouselItem.cdnUrlFieldName,
                },
