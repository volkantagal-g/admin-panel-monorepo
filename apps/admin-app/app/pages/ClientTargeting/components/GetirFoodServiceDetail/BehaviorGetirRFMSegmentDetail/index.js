...
import { getClientListData, getRFMSegmentsSelector } from '../../../redux/selectors';
import MinMaxInput from '../../common/MinMaxInput';
import RFMSegmentsSelect from '../../common/RFMSegmentsSelect';
import { Creators } from '../../../redux/actions';

const subSectionName = 'behaviorRFTMSegment';
const activeKey = `${clientListSections.getirFoodServiceDetail}.${subSectionName}`;

const BehaviorGetirRFMSegmentDetail = () => {
  const { t } = useTranslation('clientTargetingPage');
  const clientListData = useSelector(getClientListData(activeKey));
  const rfmSegments = useSelector(getRFMSegmentsSelector.getData);
  const isRfmSegmentsPending = useSelector(getRFMSegmentsSelector.getIsPending);
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(Creators.getRFMSegmentsRequest());
  }, [dispatch]);
  ...
  return (
    <CollapsePanel header={t("BEHAVIOR_GF_RFM_SEGMENT_DETAIL")} activeKey={activeKey}>
      <ActiveParamButtons 
        activeKey={activeKey}
      ...
      <RFMSegmentsSelect
        ...
        selectable={rfmSegments}
        disabled={isRfmSegmentsPending}
      />
    </CollapsePanel>
  );
};

export default BehaviorGetirRFMSegmentDetail;
