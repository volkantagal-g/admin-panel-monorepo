...
const subSectionName = 'favoriteDayOfTheWeek';
const activeKey = `${clientListSections.getirDriveServiceDetail}.${subSectionName}`;

const FavoriteDayOfTheWeekDetail = () => {
  ...
  const isoDayOptions = getISODayOptions(t);

  return (
    <CollapsePanel header={t('GETIR_DRIVE_FAVORITE_DAY_OF_THE_WEEK_DETAIL')} activeKey={activeKey}>
      <ActiveParamButtons
        activeKey={activeKey}
      ...
      <Row justify="space-between">
        <Col span={11}>
          ...
          <MultipleSelect
            activeKey={activeKey}
            value={data.favoriteDays}
            label={t('GETIR_DRIVE_FAVORITE_DAY_OF_THE_WEEK')}
            clientListKey="favoriteDays"
            selectable={isoDayOptions}
            placeholder={t('GETIR_DRIVE_FAVORITE_DAY_OF_THE_WEEK')}
          />
        </Col>
        <Col span={11}>
...
export default FavoriteDayOfTheWeekDetail;
