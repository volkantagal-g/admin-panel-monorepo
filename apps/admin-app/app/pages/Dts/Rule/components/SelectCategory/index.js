...
const SelectCategoryFormType = ({
  ...
  const categories = useSelector(categorySelector.getData);
  const isPending = useSelector(categorySelector.getIsPending);
  ...
  useEffect(() => {
    ...
    dispatch(Creators.getCategoryRequest());
  ...
  useEffect(() => {
    const options = convertSelectOptions(categories, { valueKey: '_id', labelKey: 'title', isTranslation: true });
...
const reduxKey = REDUX_KEY.DTS.RULE.SELECT_CATEGORY;
const withSaga = injectSaga({ key: reduxKey, saga });
const withReducer = injectReducer({ key: reduxKey, reducer });

export default compose(withReducer, withSaga)(SelectCategoryFormType);
