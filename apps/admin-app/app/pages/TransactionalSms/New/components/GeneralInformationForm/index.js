...
import { configSelector } from '@app/pages/TransactionalSms/New/redux/selectors';
import { Creators } from '@app/pages/TransactionalSms/New/redux/actions';
  ...
  return (
    <Badge.Ribbon text={<Ribbon />}>
      <AntCard footer={false} bordered={false} title={t('GENERAL_INFO')}>
        ...
        <CommunicationConfig configData={configData} isConfigPending={isConfigPending} componentName="TYPE" serviceType="SMS" />
        <Row gutter={24}>
          <Col md={12} xs={24}>
            <Form.Item hasFeedback name="targetService" label={t('TARGET_SERVICE')} rules={rules.onlyRequired}>
              <Select
                ...
                disabled={activeDomainsFromConfigIsPending}
        ...
        <SelectCategory serviceType={SERVICE_TYPES.SMS} />
        <SelectCallbackProperties callbackType={CALLBACK_TYPES.DATA} serviceType={SERVICE_TYPES.SMS} />
        <SelectCallbackProperties callbackType={CALLBACK_TYPES.STATUS} serviceType={SERVICE_TYPES.SMS} />
        <Row gutter={24}>
          <Col md={12} xs={24}>
            <Form.Item
              label={(
            ...
            >
              <TimePicker format={format} placeholder={t('MAX_SENDING_DURATION_PLACEHOLDER')} showNow={false} className="w-100" />
            </Form.Item>
          </Col>
        </Row>
      </AntCard>
