...
const initialUrl = '/dts/setting/category/new';

describe('In Category Setting Creation Page:', () => {
  let renderResult;
  afterAll(cleanup);
  describe('For page features', () => {
    it('should render without an error', async () => {
      renderResult = await renderPage({
        pagePermKey: permKey.PAGE_DTS_CATEGORY_SETTING_NEW,
        pageUrl: initialUrl,
    ...
    it('should have correct page content', async () => {
      expect(screen.getByText('New DTS Category')).toBeInTheDocument();
    ...
    it('should be able to save everyfield', async () => {
      ...
      act(() => {
        addUserPermissions([permKey.PAGE_DTS_CATEGORY_SETTING_LIST]);
      });
      ...
      await waitFor(() => {
        expect(screen.queryByText('Disciplinary Tracking System Category Setting List')).not.toBeInTheDocument();
      });
    });
  });
});
