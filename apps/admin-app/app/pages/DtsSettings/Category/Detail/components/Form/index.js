...
import { getDtsCategoryDetailSelector } from '../../redux/selector';
import { ACTIVENESS_OPTIONS } from '@shared/shared/constantValues';

const { Text } = Typography;

const DtsCategoryDetailForm = ({ id }) => {
  const { t } = useTranslation('dtsCategorySetting');
  const [form] = Form.useForm();
  const [isFormEditable, setIsFormEditable] = useState(false);

  const dispatch = useDispatch();
  const data = useSelector(getDtsCategoryDetailSelector.getData);
  const isPending = useSelector(getDtsCategoryDetailSelector.getIsPending);
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      dispatch(Creators.updateDtsCategorySettingDetailRequest({ data: { ...values, id } }));
    },
  });
  ...
  return (
    <Form form={form} onFinish={handleSubmit}>
      <Card
        title={t('DTS_CATEGORY_SETTING_DETAIL')}
        footer={(
          <Footer
...
export default DtsCategoryDetailForm;
