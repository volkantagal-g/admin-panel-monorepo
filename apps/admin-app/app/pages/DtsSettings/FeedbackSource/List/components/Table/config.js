...
export const tableColumns = (lang, hasAccessToCategorySettingDetailPage) => {
  const columns = [
    ...
    {
      title: t('dtsFeedbackSetting:DTS_FEEDBACK_SETTING_TITLE'),
      dataIndex: 'title',
      key: 'title',
      width: 200,
      render: title => {
    ...
    {
      title: t('dtsFeedbackSetting:DTS_FEEDBACK_SETTING_DESCRIPTION'),
  ...
  if (hasAccessToCategorySettingDetailPage) {
    columns.push({
      ...
      render: record => {
        ...
        const path = ROUTE.DTS_FEEDBACK_SOURCE_SETTING_DETAIL.path.replace(':id', '');
