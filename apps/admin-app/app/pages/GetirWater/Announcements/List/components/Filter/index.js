...
const AnnouncementFilterArea = () => {
  const dispatch = useDispatch();
  const { t } = useTranslation('getirWaterAnnouncementsPage');
  const [filters, setFilters] = useState({
    endDate: undefined,
    startDate: undefined,
    searchTextTitle: undefined,
    status: undefined,
  });

  const handleFilterClick = () => {
    dispatch(Creators.getAnnouncementsRequest({ data: filters }));
  };
  ...
  const handleChangeSearchText = e => {
    setFilters({
      ...filters,
      searchTextTitle: e.target.value,
    });
  };
  ...
  const announcementStatus = [
    ...
    { value: 'INACTIVE', label: t('global:INACTIVE') },
  ];
          ...
          <Panel header={t('FILTER')} key="1">
            <Space direction="vertical" className="w-100">
              <Row gutter={[8, 8]}>
                ...
                <Col span={8}>
                  <Text>{t('getirWaterAnnouncementsPage:COLUMNS.STATUS')}</Text>
                  <Select
                    placeholder={t(
                      'getirWaterAnnouncementsPage:COLUMNS.STATUS',
                    )}
                    className="w-100"
...
export default AnnouncementFilterArea;
