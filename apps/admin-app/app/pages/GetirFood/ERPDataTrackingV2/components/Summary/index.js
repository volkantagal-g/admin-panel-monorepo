...
import { ERPDataTrackingSummarySelector } from '../../redux/selectors';

const Summary = () => {
  const dispatch = useDispatch();
  const tableData = useSelector(ERPDataTrackingSummarySelector.getData);
  const isPending = useSelector(ERPDataTrackingSummarySelector.getIsPending);

  const getERPDataTrackingSummary = useCallback(
    dateRange => {
      if (dateRange) {
        dispatch(
          Creators.getERPDataTrackingSummaryRequest({
