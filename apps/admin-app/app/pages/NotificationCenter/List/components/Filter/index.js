...
import { FILTER_FORM } from '@app/pages/NotificationCenter/List/components/Filter/config';
import { Creators } from '@app/pages/NotificationCenter/List/redux/actions';
import { initialValues, manipulateValuesAfterSubmit } from '@app/pages/NotificationCenter/List/components/Filter/utils';
...
import { INITIAL_STATE } from '@app/pages/NotificationCenter/List/redux/reducer';
                ...
                validator(rule, clientIds) {
                  for (let i = 0; i < clientIds?.length; i += 1) {
                    if (!isObjectIdValid(clientIds[i])) {
                      return Promise.reject(t('INVALID_ANNOUNCEMENT_ID'));
                    }
                  }
...
export default memo(NotificationCenterFilter);
