...
const chartViewStatusLS = new GLocalStorage({
  key: 'SUMMARY_QUICK_CHART_VIEW_STATUSES',
...
const getGlobalDailySummaryDataSuccess = (state = INITIAL_STATE, { config, tableKey, data }) => {
  const { dataKey, expandDataKeys = [], shouldFetchExpandableRowDataBeforeExpand } = config;

  const resettedExpandDatas = {};

  // if we reset top level row data, its expand data should also reset
  if (!shouldFetchExpandableRowDataBeforeExpand) {
    expandDataKeys.forEach(expandKey => {
