import { DECIMAL_COUNT_BY_CURRENCY } from '../constants';
import {
...
export const getKuzeydenTableChartConfigs = ({ t, langKey }) => {
  const chartConfigsByDomainType = getCountsTableChartLineConfigs({ langKey });
...
export const getGetirFoodTableChartConfigs = ({ t, currency }) => {
  ...
  return [
    {
      name: `${t('GETIR_FOOD')} - ${t('dailySummaryPage:ORDER')}`,
...
export const getGetirLocalsTableChartConfigs = ({ t, currency }) => {
  ...
  return [
    {
      name: `${t('GETIR_LOCALS')} - ${t('dailySummaryPage:ORDER')}`,
