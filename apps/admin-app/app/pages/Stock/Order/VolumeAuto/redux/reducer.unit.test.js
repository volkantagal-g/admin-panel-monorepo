import reducer, { INITIAL_STATE } from '@app/pages/Stock/Order/VolumeAuto/redux/reducers';
import { Types } from '@app/pages/Stock/Order/VolumeAuto/redux/actions';

describe('Stock/Order/VolumeAuto', () => {
  it('should equal to initial state (without args)', () => {
    expect(reducer()).toEqual(INITIAL_STATE);
  });

  describe('reducer GET_SUPPLIERS_REQUEST', () => {
    it('receivedState should equal to expectedState', () => {
      const receivedState = reducer({}, { type: Types.GET_SUPPLIERS_REQUEST });
      const expectedState = { suppliers: { isPending: true } };
      expect(receivedState).toEqual(expectedState);
    });
  });

  describe('reducer GET_SUPPLIERS_SUCCESS', () => {
    it('receivedState should equal to expectedState', () => {
      const receivedState = reducer({}, { type: Types.GET_SUPPLIERS_SUCCESS, data: { id: '123' } });
      const expectedState = {
        suppliers: {
          isPending: false,
          data: { id: '123' },
  ...
  describe('reducer GET_SUPPLIERS_FAILURE', () => {
    it('receivedState should equal to expectedState', () => {
      const receivedState = reducer({}, { type: Types.GET_SUPPLIERS_FAILURE, error: new Error('404 Not Found') });
      const expectedState = {
        suppliers: {
          isPending: false,
