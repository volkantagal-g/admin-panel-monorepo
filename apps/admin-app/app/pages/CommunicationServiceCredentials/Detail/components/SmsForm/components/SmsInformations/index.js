...
import { configSelector, providerSelector } from '@app/pages/CommunicationServiceCredentials/Detail/redux/selectors';
...
const SmsInformations = ({
  ...
  useEffect(() => {
    dispatch(Creators.getConfigRequest({
      clientLanguage: getLangKey(),
      serviceType,
    }));
    Object.keys(PROVIDER_TYPE)
      .forEach(key => {
        dispatch(Creators.getProviderRequest({
          clientLanguage: getLangKey(),
          providerType: PROVIDER_TYPE[key],
        }));
      });
  ...
  return (
    <>
              ...
              <Col md={12} xs={12}>
                <Form.Item hasFeedback name={name} label={t(`${label}`)} rules={rules.onlyRequired}>
                  <Select
                    ...
                    disabled={isProviderPending || !isFormEditable}
                ...
                {({ getFieldValue }) => {
                  ...
                  return (
                    dependentComponentName !== undefined ? (
                      <Col md={12} xs={12}>
                        <Form.Item hasFeedback name={dependentComponentName} label={t(dependentComponentLabel)} rules={rules.onlyRequired}>
                          <Select
                            loading={isProviderPending}
                            disabled={isProviderPending || !isFormEditable}
      ...
      <MixpanelDataFeedFlag disabled={!isFormEditable} />
    </>
  );
};
