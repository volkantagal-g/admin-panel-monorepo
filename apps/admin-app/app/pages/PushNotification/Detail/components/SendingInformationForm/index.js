...
import { Badge, Col, DatePicker, Form, Input, InputNumber, Row, Select, Space, Tooltip } from 'antd';
...
import {
  DAYS_OF_WEEKS,
  MINUTE_IN_HOUR,
  NOTIFICATION_DATE_FORMAT, NOTIFICATION_OPTION_TYPES,
...
const SendingInformationForm = ({ form, isFormEditable }) => {
  const disabledDate = current => {
    // Can not select date before now
              ...
              rules={[
                ({ getFieldValue }) => ({
                  validator(_, validDates) {
                    const processStatus = getFieldValue('processStatus');
                    ...
                    if (processStatus !== NOTIFICATION_PROCESS_STATUS.DAILY_FINISHED) {
                  ...
                  <Row gutter={24}>
                    <Col xs={24} sm={12} lg={12}>
                      <Form.Item name="daysOfWeek" label={t('DAILY_SCHEDULER')}>
                        <MatrixCellPicker
                          activeCells={form.getFieldValue('daysOfWeek')}
                          colLabels={weekdays}
                  ...
                  || NOTIFICATION_SEND_TYPE.PARTIAL === sendingType) && (
                    <Col xs={24} md={6}>
                      <Form.Item
                        ...
                        rules={onlyRequired}
                      >
                        <InputNumber disabled={!isFormEditable} controls={false} className="w-100" min={0} />
                      </Form.Item>
                    </Col>
                )}
                  ...
                  || NOTIFICATION_SEND_TYPE.PARTIAL === sendingType) && (
                    <Col xs={24} md={6} className="mt-2 mt-md-0">
                      <Form.Item
                        ...
                        rules={onlyRequired}
                      >
                        <InputNumber disabled={!isFormEditable} controls={false} className="w-100" min={0} max={10} />
                      </Form.Item>
                    </Col>
                )}
                ...
                {NOTIFICATION_SEND_TYPE.PARTIAL === sendingType && (
                  <Col xs={24} md={6} className="mt-2 mt-md-0">
                    <Form.Item
                      ...
                      rules={onlyRequired}
                    >
                      <Select disabled={!isFormEditable} options={convertConstantValuesToSelectOptions(notificationPartialTimeInterval)} />
                    </Form.Item>
                  </Col>
                )}
