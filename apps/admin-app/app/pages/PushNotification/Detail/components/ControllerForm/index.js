...
import StockLevelControllerForm from '@app/pages/PushNotification/Detail/components/ControllerForm/components/StockLevelControllerForm';
import AggressionLevelControllerForm from '@app/pages/PushNotification/Detail/components/ControllerForm/components/AggressionLevelControllerForm';
import CityControllerForm from '@app/pages/PushNotification/Detail/components/ControllerForm/components/CityControllerForm';

const ControllerForm = ({ form, isFormEditable }) => {
              ...
              <Checkbox.Group
                ...
                onChange={checkedValue => {
                  ...
                  if (!checkedValue.includes(CONTROLLER.LOCATION_BASED_CONTROLLER)) {
                    form.setFieldsValue({ controls: { cityAndWarehouseControl: null } });
                  }
                }}
            ...
            return (
              <Space direction="vertical" className="w-100">
                {controlType.includes(CONTROLLER.STOCK_LEVEL_CONTROLLER) && isActiveServiceAppropriateForController(domainType) && (
                  <StockLevelControllerForm form={form} controlFormName={controlFormName} isFormEditable={isFormEditable} />
                )}
                {controlType.includes(CONTROLLER.AGGRESSION_LEVEL_CONTROLLER) && isActiveServiceAppropriateForController(domainType) && (
                  <AggressionLevelControllerForm controlFormName={controlFormName} isFormEditable={isFormEditable} />
                )}
                {controlType.includes(CONTROLLER.LOCATION_BASED_CONTROLLER) && (
                  <CityControllerForm form={form} controlFormName={controlFormName} isFormEditable={isFormEditable} />
                )}
              </Space>
            );
          }
