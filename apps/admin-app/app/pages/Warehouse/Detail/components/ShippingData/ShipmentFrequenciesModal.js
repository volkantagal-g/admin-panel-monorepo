...
import { defaultFrequencyValues, validationFrequencySchema } from './formHelper';
...
export default function ShipmentFrequenciesModal({ isModalVisible, closeModal, submitRequest, data }) {
  const { t } = useTranslation();
  const [isValidationModalVisible, setIsValidationModalVisible] = useState(false);
  const [form] = useForm();
  const formik = useFormik({
    initialValues: defaultFrequencyValues,
    validate: validate(validationFrequencySchema),
    onSubmit: values => {
      if (_.isEqual(values, defaultFrequencyValues)) {
        return;
      }
  ...
  const frequencyCodes = useSelector(warehouseSelector.getShipmentFrequencies);
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      setValues(defaultFrequencyValues);
    }
        ...
        <Row gutter={[16]} align="bottom">
          ...
          <Col span={12}>
            <DatePickerWrapper
              ...
              value={values.from ? moment(values.from) : undefined}
