...
import { Col, DatePicker, Form, InputNumber, Row, Select, Tooltip } from 'antd';
      ...
      <Row gutter={24} className="mt-3">
        <Col lg={12}>
          <Form.Item name="status" label={t('STATUS')} rules={rules.onlyRequired}>
            <Select disabled={!isFormEditable} options={convertConstantValuesToSelectOptions(popupStatusTypeOptions)} />
          </Form.Item>
        </Col>
      </Row>
      <Row gutter={24}>
      ...
      <Row gutter={24}>
        <Col lg={12}>
          <Form.Item name="type" label={t('POPUP_TYPE')} rules={rules.onlyRequired}>
            <Select disabled={!isFormEditable}>
        ...
        {/* Promo selection Begin */}
        <Col lg={12}>
          {/* Form item will update only type & promo error changed */}
              ...
              if (POPUP_TYPE.PROMO === popupType) {
                return (
                  <PromotionSelect
                    ...
                    inline
                    rules={rules.onlyRequired}
                    disabled={!targetDomainType || !isFormEditable}
                  />
                );
              }
              return null;
      ...
      <Row gutter={24}>
        <Col lg={12}>
          <Form.Item
            ...
            rules={rules.onlyRequired}
          >
            <InputNumber disabled={!isFormEditable} controls={false} className="w-100" min={1} max={1000} />
          </Form.Item>
        </Col>
      </Row>
      <Row gutter={24}>
        <Col lg={12}>
          <SegmentSelect label={t('SEGMENTS')} fieldName="clientSegments" mode="multiple" rules={rules.requiredArray} disabled={!isFormEditable} />
        </Col>
        <Col lg={12}>
          <SegmentSelect label={t('EXCLUDED_SEGMENT')} fieldName="clientExSegments" mode="multiple" inline disabled={!isFormEditable} />
        </Col>
      </Row>
      <Row gutter={24}>
        <Col lg={12}>
