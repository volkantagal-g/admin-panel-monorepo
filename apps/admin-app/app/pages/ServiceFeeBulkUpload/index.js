...
const ServiceFeeBulkUpload = () => {
  usePageViewAnalytics({ name: ROUTE.SERVICE_FEE_BULK_UPLOAD.name, squad: ROUTE.SERVICE_FEE_BULK_UPLOAD.squad });
  ...
  const pageTitle = t('PAGE_TITLE.SERVICE_FEE.BULK_UPLOAD');
  ...
  return (
    <Row gutter={[8, 8]} align="middle">
      ...
      <ImportCsv
        title={t('PAGE_TITLE.SERVICE_FEE.BULK_UPLOAD')}
        mode={FEE_LAYER_TYPE.REGULAR}
        exampleCsvUrl={exampleCsvUrl}
      />
      <ImportCsv
        title={t('PAGE_TITLE.SERVICE_FEE.BULK_UPLOAD_PEAK_HOURS')}
...
const reduxKey = REDUX_KEY.SERVICE_FEE.BULK_UPLOAD;
const withSaga = injectSaga({ key: reduxKey, saga: sagas });
const withReducer = injectReducer({ key: reduxKey, reducer });

export default compose(withReducer, withSaga)(ServiceFeeBulkUpload);
