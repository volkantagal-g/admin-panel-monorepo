 ...
 * @returns {Array}
 */
const getDefaultTypeValidations = (type, name, isEdit) => {
  switch (type) {
    case 'objectId':
      return [
        {
    ...
    case 'date':
      if (!isEdit && (name === mandatoryDateFields.startDate || name === mandatoryDateFields.endDate)) {
        return [
          {
            type: 'min',
            params: [
 ...
 * }
 */
export const createYupSchema = (fields, isEdit = false) => {
  const schema = {};

  fields.forEach(field => {
    const { isRequired, name, type: fieldType, validations: fieldValidations } = field;

    const validations = getDefaultTypeValidations(fieldType, name, isEdit);
    ...
    let validator = {
      boolean: Yup.boolean(),
      date: Yup.date(),
      integer: Yup.number().integer().nullable(true),
      objectId: Yup.string(),
