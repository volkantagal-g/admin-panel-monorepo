...
import { getInitialValues, teamSchema } from './formHelper';
...
const TeamDetail = () => {
  ...
  const isUpdating = useSelector(updateTeamSelector.getIsPending);
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      ...
      dispatch(Creators.updateTeamRequest({
        team: {
          ...values,
          id,
        },
  ...
  const handleCancelClick = () => {
    setValues(getInitialValues(team));
    setIsFormEditable(false);
  };
  ...
  const cardExtra = (
    <Tooltip title={t('GENERAL_INFO_LAST_UPDATE')}>
      <Space>
        <EditOutlined />
        <Text>{moment(team?.updatedAt).format(getLocalDateTimeFormat())}</Text>
      </Space>
    </Tooltip>
  );
