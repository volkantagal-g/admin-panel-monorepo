...
import { Button, Form, Input, Row, Col, Select, DatePicker, Checkbox, Tag } from 'antd';
...
import { getInitialValues, AssetSchemaV1, AssetSchemaV2, manipulateValuesAfterSubmit } from './formHelper';
import AntCard from '@shared/components/UI/AntCard';
import { validate } from '@shared/yup';
import { DEFAULT_DATE_FORMAT, TAG_COLORS } from '@shared/shared/constants';
...
const AssetDetailForm = () => {
  ...
  const { t } = useTranslation(['assetPage', 'global', 'assetManagement']);
  ...
  const [assetCategory, setAssetCategory] = useState(ASSET_CATEGORIES.IT);

  const getAssetDetailData = useSelector(AssetDetailSelector.getAssetDetailData);
  const isPending = useSelector(AssetDetailSelector.getAssetDetailIsPending);

  const validationSchema = (
    getAssetDetailData.version &&
      getAssetDetailData.version === ASSET_VERSIONS.v2 &&
      getAssetDetailData.assetCategory === ASSET_CATEGORIES.IT
  ) ? AssetSchemaV2 : AssetSchemaV1;
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      dispatch(Creators.updateAssetRequest({
        body: {
          ...manipulateValuesAfterSubmit(values, assetCategory),
          assignedEmployee: get(values.assignedEmployee, 'value', values.assignedEmployee),
        },
        assetId,
      }));
  ...
  const { handleSubmit, values, errors, setFieldValue, touched, setValues } = formik;
  ...
  useEffect(() => {
    dispatch(Creators.assetDetailRequest({ assetId }));
  }, [dispatch, assetId]);

  const handleCancelClick = () => {
    setValues(getInitialValues(getAssetDetailData));
    setIsFormEditable(false);
  };
  ...
  useEffect(() => {
    setValues(getInitialValues(getAssetDetailData));
    setAssetCategory(getAssetDetailData.assetCategory ?? 1);
  }, [form, getAssetDetailData, setValues]);
  ...
  const getHandleChange = (fieldName, inputType = 'text') => {
    return param => {
      ...
      if (fieldName === 'deviceType') {
        setFieldValue('brand', '');
        ASSET_DEVICE_CONFIG_FIELDS.forEach(field => {
          setFieldValue(field, '');
        });
      }
  ...
  return (
    <AntCard
      ...
      title={t('SUB_HEADING')}
    >
      {!isEmpty(getAssetDetailData?.assignedEmployee) ? (
        <Tag className={classes.assignmentStatusTag} color={TAG_COLORS.success}>
          {t('assetManagement:ASSIGNED')}
        </Tag>
      ) :
        (
          <Tag className={classes.assignmentStatusTag} color={TAG_COLORS.danger}>
            {t('assetManagement:UNASSIGNED')}
          </Tag>
        )}
      <Form form={form} id="user-new" onFinish={handleSubmit} layout="vertical">
        <Row gutter={[theme.spacing(3)]} align="bottom">
          ...
          <Col span={12}>
            ...
            >
              <Checkbox
                checked={get(values, 'mdmInstalled', false)}
                onChange={getHandleChange('mdmInstalled', 'checkbox')}
                disabled={isPending || !isFormEditable}
              />
            </Form.Item>
          </Col>
          ...
          <Col span={12}>
            ...
            >
              <Checkbox
                ...
                disabled={isPending || !isFormEditable || values.latestAssignDate}
              />
            </Form.Item>
          </Col>
          ...
          <Col span={12}>
            <Form.Item
              ...
              validateStatus={get(errors, 'deviceSerialNumber') ? 'error' : 'success'}
          ...
          <Col span={12}>
            <Form.Item
              ...
              validateStatus={get(errors, 'invoiceNumber') ? 'error' : 'success'}
              name={['invoiceNumber']}
          ...
          <Col span={12}>
            <Form.Item
              ...
              validateStatus={get(errors, 'invoiceDate') ? 'error' : 'success'}
              name={['invoiceDate']}
          ...
          <Col span={12}>
            <Form.Item
              ...
              validateStatus={get(errors, 'supplier') ? 'error' : 'success'}
              name={['supplier']}
              label={t('SUPPLIER')}
            >
              <Input
          ...
          <Col span={12}>
            <Form.Item
              ...
              validateStatus={touched.type && get(errors, 'country') ? 'error' : 'success'}
              name={['country']}
              label={t('COUNTRY')}
            >
              <Select
          ...
          <Col span={12}>
            <Form.Item
              ...
              validateStatus={touched.type && get(errors, 'city') ? 'error' : 'success'}
              name={['city']}
              label={t('CITY')}
            >
              <Select
                ...
                filterOption
                optionFilterProp="label"
                disabled={isPending || !isFormEditable || values.assignedEmployee}
                value={values.city}
          ...
          <Col span={12}>
            <Form.Item
              ...
              validateStatus={touched.type && get(errors, 'deviceStatus') ? 'error' : 'success'}
              name={['deviceStatus']}
          ...
          <Col span={12}>
            <Form.Item
              ...
              validateStatus={touched.type && get(errors, 'assignmentStatus') ? 'error' : 'success'}
              ...
              <>
                <Col span={12}>
                  <Form.Item
                    ...
                    validateStatus={get(errors, 'warrantyEndDate') ? 'error' : 'success'}
          ...
          <Col span={12}>
            <Form.Item
              name={['createdAt']}
              label={t('ASSET_CREATION_DATE')}
            >
              <DatePicker
                value={values.createdAt}
                onChange={getHandleChange('createdAt')}
                format={DEFAULT_DATE_FORMAT}
                placeholder={t('ASSET_CREATION_DATE')}
                className={classes.datePickerWidth}
                disabled
              />
            </Form.Item>
                      ...
                      <Col key={+index} span={12}>
                        ...
                        >
                          <Input
                            ...
                            autoComplete="off"
                            disabled={isPending || !isFormEditable}
                            defaultValue={values.macAddresses[index]}
                          />
                        </Form.Item>
                      </Col>
                    );
          ...
          <Col span={12}>
            <Form.Item
              help={get(errors, 'assignedEmployee')}
              validateStatus={get(errors, 'assignedEmployee') ? 'error' : 'success'}
            ...
            >
              <SelectEmployee
                ...
                allowClear={false}
                disabled
                value={get(values, 'assignedEmployee')}
                placeholder={t('ASSIGNED_EMPLOYEE')}
