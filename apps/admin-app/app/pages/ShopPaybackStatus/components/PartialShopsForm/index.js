...
import { Form, Input, DatePicker, Select, Button, Modal, Alert } from 'antd';
...
import {
  REASON_AREA_MAX_CHAR_LIMIT,
  ACTION_TYPES,
  ACTION_TYPE_OPTIONS,
  MARKETPLACE_API_DATE_FORMAT,
  EXCEL_TEMPLATE_URL,
} from '@app/pages/ShopPaybackStatus/constants';
import { Creators } from '@app/pages/ShopPaybackStatus/redux/actions';
import { validatePartialShopsPaybackStatus, updatePartialShopsPaybackStatus } from '@app/pages/ShopPaybackStatus/redux/selectors';
import useStyles from './styles';

const PartialShopsForm = () => {
  ...
  const isUpdateStatusPending = useSelector(updatePartialShopsPaybackStatus.getIsPending);
  const isValidateExcelPending = useSelector(validatePartialShopsPaybackStatus.getIsPending);

  const handleSubmit = values => {
    dispatch(Creators.updatePartialShopsPaybackStatusRequest({
      data: {
        fileName,
        status: actionType,
  ...
  const handleConfirmSubmit = values => {
    Modal.confirm({
      ...
      content: actionType === ACTION_TYPES.PAUSE
        ? t('ACTIVE_PARTIAL_SHOPS_CONFIRM_CONTENT', { shopCount })
        : t('PAUSED_PARTIAL_SHOPS_CONFIRM_CONTENT', { shopCount }),
      icon: null,
  ...
  const handleFileChange = selectedFiles => {
    ...
    if (!isEmpty(selectedFiles)) {
      ...
      getBase64(selectedFile, base64File => {
        dispatch(Creators.validatePartialShopsPaybackStatusRequest({
  ...
  const getValidationHintLabel = () => {
    ...
    return t('UPLOAD_HINT_LABEL', { shopCount });
  };
    ...
    >
      <Form.Item rules={[{ required: true }]} label={t('PAUSE_RESUME')}>
        <Select
          ...
          options={convertConstantValuesToSelectOptions(ACTION_TYPE_OPTIONS)}
        />
      </Form.Item>
      {!isNil(actionType) && (
        <>
          ...
          <Can permKey={permKey.PAGE_GL_SHOP_PAYBACK_STATUS_COMPONENT_UPDATE}>
            <Form.Item className={classes.submitButton}>
              <Button
                ...
                loading={isUpdateStatusPending || isValidateExcelPending}
                type="primary"
                htmlType="submit"
              >
                {
...
export default PartialShopsForm;
