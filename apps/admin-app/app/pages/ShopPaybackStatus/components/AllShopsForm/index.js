import { useState } from 'react';
import { useSelector, useDispatch } from 'react-redux';
import moment from 'moment';
import { isNil } from 'lodash';
import { Form, Input, DatePicker, Select, Button, Modal } from 'antd';
import { useTranslation } from 'react-i18next';

import { getLocalDateFormat } from '@shared/utils/localization';
import { usePermission } from '@shared/hooks';
import permKey from '@shared/shared/permKey.json';
import { convertConstantValuesToSelectOptions } from '@shared/utils/common';

import {
  REASON_AREA_MAX_CHAR_LIMIT,
  ACTION_TYPE_OPTIONS,
  ACTION_TYPES,
  MARKETPLACE_API_DATE_FORMAT,
} from '@app/pages/ShopPaybackStatus/constants';
import { Creators } from '@app/pages/ShopPaybackStatus/redux/actions';
import { updateAllShopsPaybackStatus } from '@app/pages/ShopPaybackStatus/redux/selectors';
import useStyles from './styles';

const AllShopsForm = () => {
  const { t } = useTranslation('shopPaybackStatus');
  const classes = useStyles();
  const dispatch = useDispatch();
  const [form] = Form.useForm();
  const { Can } = usePermission();

  const [actionType, setActionType] = useState();

  const isUpdateAllShopsPaybackStatusPending = useSelector(updateAllShopsPaybackStatus.getIsPending);

  const handleSubmit = values => {
    dispatch(Creators.updateAllShopsPaybackStatusRequest({
      data: {
        status: actionType,
        reason: values.reason,
        pausedUntil: values.pausedUntil && moment(values.pausedUntil).format(MARKETPLACE_API_DATE_FORMAT),
      },
    }));
  };

  const handleConfirmSubmit = values => {
    Modal.confirm({
      title: t('CONFIRM_MODAL_TITLE'),
      content: actionType === ACTION_TYPES.PAUSE
        ? t('ACTIVE_ALL_SHOPS_CONFIRM_CONTENT')
        : t('PAUSED_ALL_SHOPS_CONFIRM_CONTENT'),
      icon: null,
      okText: t('global:CONFIRM'),
      cancelText: t('global:CANCEL'),
      onOk: () => handleSubmit(values),
      centered: true,
    });
  };

  const disabledDate = current => {
    return current && current < moment().endOf('day');
  };

  return (
    <Form
      form={form}
      labelCol={{ span: 8 }}
      wrapperCol={{ span: 16 }}
      labelAlign="left"
      onFinish={handleConfirmSubmit}
      colon={false}
      requiredMark={false}
    >
      <Form.Item rules={[{ required: true }]} label={t('PAUSE_RESUME')} name="actionType">
        <Select
          value={actionType}
          onChange={setActionType}
          options={convertConstantValuesToSelectOptions(ACTION_TYPE_OPTIONS)}
        />
      </Form.Item>
      {!isNil(actionType) && (
        <>
          {actionType === ACTION_TYPES.PAUSE && (
            <Form.Item
              name="pausedUntil"
              label={t('PAUSE_UNTIL')}
            >
              <DatePicker
                disabledDate={disabledDate}
                format={getLocalDateFormat()}
              />
            </Form.Item>
          )}
          <Form.Item rules={[{ required: true }]} name="reason" label={t('REASON')}>
            <Input.TextArea showCount maxLength={REASON_AREA_MAX_CHAR_LIMIT} autoSize={{ minRows: 4 }} />
          </Form.Item>
          <Can permKey={permKey.PAGE_GL_SHOP_PAYBACK_STATUS_COMPONENT_UPDATE}>
            <Form.Item className={classes.submitButton}>
              <Button loading={isUpdateAllShopsPaybackStatusPending} type="primary" htmlType="submit">
                {
                  actionType === ACTION_TYPES.PAUSE
                    ? t('PAUSE')
                    : t('RESUME')
                }
              </Button>
            </Form.Item>
          </Can>
        </>
      )}
    </Form>
  );
};

export default AllShopsForm;
