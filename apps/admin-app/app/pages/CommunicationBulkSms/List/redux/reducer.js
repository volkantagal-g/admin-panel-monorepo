...
import { Types } from '@app/pages/CommunicationBulkSms/List/redux/actions';
...
export const getCancelRequest = (state = INITIAL_STATE) => {
  return {
    ...state,
    results: {
      ...state.results,
...
export const getCancelSuccess = (state = INITIAL_STATE, { id }) => {
  const updatedContent = state.results.data.content
    .map(item => (item.id === id ? {
      ...item,
...
export const getCancelFailure = (state = INITIAL_STATE, { error }) => {
  return {
    ...state,
    results: {
      ...state.results,
...
export const HANDLERS = {
  ...
  [Types.GET_CANCEL_REQUEST]: getCancelRequest,
  [Types.GET_CANCEL_SUCCESS]: getCancelSuccess,
  [Types.GET_CANCEL_FAILURE]: getCancelFailure,
