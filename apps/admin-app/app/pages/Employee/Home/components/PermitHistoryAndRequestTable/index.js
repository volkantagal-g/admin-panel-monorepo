...
import PermitRequestTable from '../PermitRequestTable';
import PermitMyTeamTable from '@app/pages/Employee/Home/components/PermitMyTeamTable';
import { EMPLOYEE_HOME_PAGE_LEAVE_SECTION_TABS } from '../../constants';
import { FEATURE_FLAGS } from '../../../constants';
import { validatePermitPayrollCountry, convertPermitTypeConstantsToSelectOptions } from '../../utils';
...
const PermitHistoryAndRequestTable = ({ onPermitDetailClick, onNewPermitRequestBtnClick }) => {
  ...
  const { isValid: isPermitRequestValid } = validatePermitPayrollCountry();
  ...
  const contentMap = {
    [EMPLOYEE_HOME_PAGE_LEAVE_SECTION_TABS.MY_LEAVES_TAB]: <PermitHistoryTable onPermitDetailClick={onPermitDetailClick} />,
    [EMPLOYEE_HOME_PAGE_LEAVE_SECTION_TABS.MY_TEAM_TAB]: <PermitMyTeamTable />,
    [EMPLOYEE_HOME_PAGE_LEAVE_SECTION_TABS.MANAGER_VIEW_TAB]: <PermitRequestTable
  ...
  const tabBarExtraContent = useMemo(() => {
    const newLeaveRequestButton = (
      <Button
        size="small"
        type="primary"
        icon={<PlusOutlined />}
        onClick={() => onNewPermitRequestBtnClick('REGULAR_LEAVE')}
        disabled={!isPermitRequestValid}
      >
        {t('employeePage:NEW_PERMIT_REQUEST')}
      </Button>
    );
    const leaveRequestButton = !isPermitRequestValid ? (
      <Tooltip title={t('NEW_LEAVE_REQUEST_DISABLE_MESSAGE')}>
    ...
    const newOffSiteWorkRequestButton = (
      <Button
        size="small"
        type="primary"
        icon={<PlusOutlined />}
        onClick={() => onNewPermitRequestBtnClick('OFF_SITE_WORK')}
        disabled={!isPermitRequestValid}
      >
        {t('employeePage:NEW_OFF_SITE_WORK_REQUEST')}
      </Button>
    );
    const offSiteWorkRequestButton = !isPermitRequestValid ? (
      <Tooltip title={t('NEW_OFF_SITE_WORK_REQUEST_DISABLE_MESSAGE')}>
        {newOffSiteWorkRequestButton}
      </Tooltip>
    ) : (
      newOffSiteWorkRequestButton
    );

    const hasRegularLeaveOption = !!convertPermitTypeConstantsToSelectOptions({ permitRequestGroup: 'REGULAR_LEAVE' }).length;
    ...
    return (
      <div className={`${isMobile() ? 'd-block' : 'd-flex'} align-items-center`}>
        ...
        <div>
          {
            FEATURE_FLAGS.REGULAR_LEAVE_REQUESTS_ARE_ENABLED && hasRegularLeaveOption ?
              leaveRequestButton :
              <MigrationMessage message={t('employeePage:LEAVE_MANAGEMENT_WORKDAY_MIGRATION_WARNING_TEXT')} />
          }
          {/* only use nbsp if any of the features are enabled */}
          { FEATURE_FLAGS.REGULAR_LEAVE_REQUESTS_ARE_ENABLED && FEATURE_FLAGS.OFF_SITE_WORK_REQUESTS_ARE_ENABLED ? '\u00A0' : ''}
          {
            FEATURE_FLAGS.OFF_SITE_WORK_REQUESTS_ARE_ENABLED ?
              offSiteWorkRequestButton :
              <MigrationMessage message={t('employeePage:OFF_SITE_WORK_REQUEST_MANAGEMENT_WORKDAY_MIGRATION_WARNING_TEXT')} />
          }
        </div>
      </div>
    );
  }, [onNewPermitRequestBtnClick, isPermitRequestValid, isUsedAndVestedPermitCountsPending, usedAndVestedPermitCounts, view, t]);
...
export default PermitHistoryAndRequestTable;
