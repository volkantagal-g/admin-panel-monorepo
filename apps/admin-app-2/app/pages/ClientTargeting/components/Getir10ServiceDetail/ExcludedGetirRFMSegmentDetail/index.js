...
const subSectionName = 'excludedRFTMSegment';
const activeKey = `${clientListSections.getir10ServiceDetail}.${subSectionName}`;

const ExcludedGetirRFMSegmentDetail = () => {
  ...
  return (
    <CollapsePanel header={t("EXCLUDED_G10_RFM_SEGMENT_DETAIL")} activeKey={activeKey}>
      <ActiveParamButtons 
        activeKey={activeKey}
      ...
      <SegmentListSelect 
        activeKey={activeKey} 
        value={data.segments}
        label={t("EXCLUDED_RFM_SEGMENTS")}
        description={t("EXCLUDED_RFM_SEGMENTS_DESCRIPTION")}
      />
    </CollapsePanel>
  );
};

export default ExcludedGetirRFMSegmentDetail;
