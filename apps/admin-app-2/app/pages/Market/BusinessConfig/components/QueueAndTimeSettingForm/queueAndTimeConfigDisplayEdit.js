...
const QueueAndTimeConfigDisplayEdit = ({
  ...
  const onCountryValueChange = useCallback(value => {
    const [selectedCountryFormLabel] = languages;
    setFieldValue(`${selectedCountryFormLabel}`, value);
  }, [languages, setFieldValue]);

  const onGlobalValueChange = useCallback(value => {
          ...
          <Col span={24}>
            {isFormGlobal && (
              <>
                ...
                <Form.Item
                  help={get(errors, 'value')}
                  validateStatus={get(errors, 'value') ? 'error' : 'success'}
                  label={t('GLOBAL_VALUE')}
                >
                  <InputNumber
                    value={values.value}
...
export default QueueAndTimeConfigDisplayEdit;
