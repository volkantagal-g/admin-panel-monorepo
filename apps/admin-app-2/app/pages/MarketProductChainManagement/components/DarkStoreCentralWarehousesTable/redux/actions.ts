import type { CentralWarehouse, CentralWarehouseFilters, FilterOption, PaginationParams, SortParams, TableParams } from '../types';
import { ACTION_TYPES } from './constants';

const {
  FETCH_CENTRAL_WAREHOUSES_REQUEST,
  FETCH_CENTRAL_WAREHOUSES_SUCCESS,
  FETCH_CENTRAL_WAREHOUSES_FAILURE,
...
export const fetchCentralWarehousesRequest = (params: TableParams) => ({
  type: FETCH_CENTRAL_WAREHOUSES_REQUEST,
  payload: params,
});

export const fetchCentralWarehousesSuccess = (items: CentralWarehouse[], totalCount: number) => ({
  type: FETCH_CENTRAL_WAREHOUSES_SUCCESS,
  payload: { items, totalCount },
});

export const fetchCentralWarehousesFailure = (error: Error) => ({
  type: FETCH_CENTRAL_WAREHOUSES_FAILURE,
  payload: error,
});
